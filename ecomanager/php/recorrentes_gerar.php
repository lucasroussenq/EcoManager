...
try {
  while ($r = $res->fetch_assoc()) {
    ...
    if ($per === 'SEMANAL') {
      $stmtUp = $conn->prepare("UPDATE recorrente SET prox_data = DATE_ADD(prox_data, INTERVAL 7 DAY) WHERE id_recorrente = ?");
    } else {
      $stmtUp = $conn->prepare("UPDATE recorrente SET prox_data = DATE_ADD(prox_data, INTERVAL 1 MONTH) WHERE id_recorrente = ?");
    }
