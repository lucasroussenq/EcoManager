...
if ($id) {
  $stmt = $conn->prepare("UPDATE categoria SET nome=?, id_natureza=? WHERE id_categoria=? AND id_usuario=?");
  $stmt->bind_param("siii", $nome, $id_natureza, $id, $_SESSION['id_usuario']);
  $ok = $stmt->execute();
} else {
  $stmt = $conn->prepare("INSERT INTO categoria (id_usuario, id_natureza, nome) VALUES (?,?,?)");
