...
async function fetchData(){
  const r = await fetch(`../php/get_grafico_mensal.php?mes=${encodeURIComponent(mesInp.value||'')}`);
  const j = await r.json();
  if (!j.ok) throw new Error(j.msg||'Erro dados');
  // retorna {receita: number, despesa: number}
  return j;
}
function drawBars(rec, desp){
  ...
  // Receita
  cx.fillStyle = 'hsl(160 70% 50%)';
  cx.fillRect(baseX, baseY - hR, w, hR);
  cx.fillStyle='#dbe7ff'; cx.font='14px sans-serif';
  cx.fillText(`Receitas: R$ ${rec.toFixed(2)}`, baseX-20, baseY - hR - 8);

  // Despesa
  cx.fillStyle = 'hsl(12 80% 55%)';
  cx.fillRect(baseX + w + gap, baseY - hD, w, hD);
  cx.fillStyle='#dbe7ff';
...
async function desenhar(){
  try {
    ...
    drawBars(Number(d.receita||0), Number(d.despesa||0));
