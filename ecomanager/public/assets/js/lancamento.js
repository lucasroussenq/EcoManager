// public/lancamento/js/lancamento.js
...
$btnSair.addEventListener('click', async ()=>{
  try{
    const r = await apiFetch('/auth/logout.php', { method:'POST' });
    const j = await r.json();
    if (j.ok) location.replace('/ecomanager/public/auth/login.html');
    else alert(j.msg || 'Erro ao sair.');
  }catch(e){ alert('Falha ao sair: '+e.message); }
});
